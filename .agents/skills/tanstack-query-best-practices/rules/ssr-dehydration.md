# ssr-dehydration: Use dehydrate/HydrationBoundary for SSR

## Priority: MEDIUM

## Explanation

For server-side rendering, prefetch queries on the server, dehydrate the cache to a serializable format, and rehydrate it on the client with `HydrationBoundary` (or `hydrate()` in manual setups). This prevents content flash and duplicate requests.
